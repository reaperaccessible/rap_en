...
function disableAllFXExeptInstrument(track)
    ...
    reaper.osara_outputMessage("FX bypassed")
end
...
function enableAllFXExeptInstrument(track)
    ...
    reaper.osara_outputMessage("FX active")
end
...
if fxNumber < 1 then
    reaper.osara_outputMessage("No FX on this track")
    return
end
...
-- Dans le cas contraire, on active tous les effets, sauf les instruments virtuels
  ...
  reaper.Undo_EndBlock("Bypass FX of selected track, except virtual instruments", 0) 
