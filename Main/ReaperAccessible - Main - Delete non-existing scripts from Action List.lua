...
function main()
    ...
    if idx > 0 then
        ...
        local ret = reaper.MB("Do you want to delete"..idx.." record(s) in reaper-kb.ini?\n" ,"Remove non-existent scripts from Actions list?", 4)
        ...
        if ret == 6 then 
            ...
            reaper.MB(idx.." record(s) deleted. \nReload REAPER to affect changes\n\n".."REAPER/reaper-kb.ini-backup is created. The list of deleted scripts is in the clipboard.", "", 0)
        ...
        reaper.MB("Nothing to delete", "Delete non-existent scripts from Action List", 0)
    end
end
