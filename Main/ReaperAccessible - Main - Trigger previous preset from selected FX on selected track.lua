-- @Description Trigger previous preset from selected FX on selected track
-- @version 1.0
...
local rpa =  reaper.NamedCommandLookup("_S&M_PREVIOUS_SELFX_PRESET")
...
idxSelFx1 = idxSelFx+1 
   ...
   retval, presetname = reaper.TrackFX_GetPreset(LastTouchedTrack, idxSelFx, "")
            if presetname == "" then presetname = "Aucun préréglage" end;




    reaper.osara_outputMessage( presetname)

  reaper.Undo_EndBlock("Triggered previous preset", 0) 
