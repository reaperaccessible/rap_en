...
local function o(e)
    ...
    reaper.Undo_BeginBlock()
    retval, csv = reaper.GetUserInputs("Minimum and maximum velocity value", 2, 'max,min', '1,127')
    if retval then
...
if retval then
    reaper.osara_outputMessage("Random velocity applied")
else
    reaper.osara_outputMessage("Canceled")
end

reaper.Main_OnCommand(40109, 0)

reaper.Undo_EndBlock("Set random velocity for selected notes, between 2 limits prompts by user", 0) 
