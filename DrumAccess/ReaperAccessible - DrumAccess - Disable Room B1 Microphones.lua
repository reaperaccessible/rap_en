-- @description Disable Room B1 Microphones for DrumAccess
-- @version 1.1
...
if keywordDetected then
    reaper.defer(function()
        reaper.osara_outputMessage("Room B1 Microphones are Disabled.")
    end)
else
    reaper.osara_outputMessage("No parameters are displayed, the selected track does not contain DrumAccess FX, or this kit piece does not offer Room B1 microphone. Please select the DrumAccess folder track, trigger the script ReaperAccessible - DrumAccess - Make drum kit parameters available, select a track that contains a DrumAccess FX, and triggering this action again.")
end